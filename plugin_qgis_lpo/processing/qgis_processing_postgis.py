...
class TableField(object):
    def __init__(
        ...
        (self.name, self.data_type, self.is_null, self.default, self.modifier) = (
            name,
            data_type,
            is_null,
            default,
    ...
    def field_def(self):
        ...
        data_type = (
            self.data_type
            if not self.modifier or self.modifier < 0
            else "%s(%d)" % (self.data_type, self.modifier)
        )
...
class GeoDB(object):
    @classmethod
    def from_name(cls, conn_name: str):
            ...
            try:
                ...
                if err is not None:
                    QgsCredentials.instance().put(
                        conninfo, self.uri.username(), self.uri.password()
                    )
                break
                ...
                if not ok:
                    raise QgsProcessingException(
                        QCoreApplication.translate("PostGIS", "Action canceled by user")
                    )
                if user:
    ...
    def check_postgis(self):
        ...
        c = self.con.cursor()
        self._exec_sql(
            c, "SELECT COUNT(*) FROM pg_proc WHERE proname = 'postgis_version'"
        )
    ...
    def list_geotables(self, schema: str = None):
    ...
    def get_table_rows(self, table: str, schema: str = None):
        c = self.con.cursor()
        self._exec_sql(c, "SELECT COUNT(*) FROM %s" % self._table_name(schema, table))
        return c.fetchone()[0]

    def get_table_fields(self, table: str, schema: str = None):
        """Return list of columns in table"""

        c = self.con.cursor()
        schema_where = (
            " AND nspname='%s' " % self._quote_unicode(schema)
    ...
    def get_table_indexes(self, table: str, schema: str = None):
    ...
    def get_table_constraints(self, table: str, schema: str = None):
        c = self.con.cursor()
    ...
    def get_view_definition(self, view: str, schema: str = None):
    ...
    def delete_geometry_column(self, table: str, geom_column: str, schema: str = None):
    ...
    def delete_geometry_table(self, table: str, schema: str = None):
        ...
        if schema:
            ...
            schema_part = ""
        sql = "SELECT DropGeometryTable(%s'%s')" % (
            schema_part,
            self._quote_unicode(table),
        )
        self._exec_sql_and_commit(sql)

    def create_table(self, table: str, fields, pkey: str = None, schema: str = None):
    ...
    def delete_table(self, table: str, schema: str = None):
    ...
    def rename_table(self, table: str, new_table: str, schema: str = None):
        ...
        if self.has_postgis:
            sql = (
                "UPDATE geometry_columns SET f_table_name='%s' \
    ...
    def create_view(self, name: str, query: str, schema: str = None):
        view_name = self._table_name(schema, name)
        sql = "CREATE VIEW %s AS %s" % (view_name, query)
        self._exec_sql_and_commit(sql)

    def delete_view(self, name: str, schema: str = None):
        view_name = self._table_name(schema, name)
        sql = "DROP VIEW %s" % view_name
        self._exec_sql_and_commit(sql)

    def rename_view(self, name: str, new_name: str, schema: str = None):
    ...
    def get_database_privileges(self):
        ...
        sql = (
            "SELECT has_database_privilege('%(d)s', 'CREATE'), \
    ...
    def sr_info_for_srid(self, srid: int) -> str:
        if not self.has_postgis:
            return "Unknown"
        ...
        try:
            c = self.con.cursor()
            self._exec_sql(
                c, "SELECT srtext FROM spatial_ref_sys WHERE srid = '%d'" % srid
            )
            srtext = c.fetchone()[0]
