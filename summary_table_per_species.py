    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        query = """WITH obs AS (
                    ...
                    data AS (
                        ...
                        LEFT JOIN taxonomie.mv_c_statut_lr lr ON (obs.taxref_cdnom, obs.nom_sci) = (lr.cd_ref, lr.vn_nom_sci)
                        LEFT JOIN taxonomie.mv_c_statut_protection p ON (obs.taxref_cdnom, obs.nom_sci) = (p.cd_ref, p.vn_nom_sci)
                        GROUP BY
                        obs.taxref_cdnom
                        , obs.groupe_taxo
                        , obs.nom_vern
