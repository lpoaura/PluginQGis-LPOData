...
SELECT
    ...
    sd.id_nomenclature_valid_status
...
FROM gn_synthese.synthese AS sd
INNER JOIN taxonomie.mv_c_cor_vn_taxref AS t ON sd.cd_nom = t.cd_nom
INNER JOIN gn_synthese.t_sources AS s ON sd.id_source = s.id_source
LEFT JOIN
    src_lpodatas.t_c_synthese_extended AS se
    ON sd.id_synthese = se.id_synthese
LEFT JOIN
    src_vn_json.forms_json AS forms
    ON se.id_form::TEXT = forms.id_form_universal::TEXT
LEFT JOIN gn_meta.t_datasets AS ds ON sd.id_dataset = ds.id_dataset
LEFT JOIN ref_nomenclatures.t_nomenclatures AS nom_valid
    ON
        sd.id_nomenclature_valid_status = nom_valid.id_nomenclature
        AND nom_valid.id_type = ref_nomenclatures.get_id_nomenclature_type('STATUT_VALID'::CHARACTER VARYING);
COMMIT;
